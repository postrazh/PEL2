...
int CInjector::Inject(std::wstring dllName, std::wstring processName, DWORD pId)
{
	HANDLE hProc = NULL, hThread = NULL;
	HMODULE hModule;
	int len;
	void* pRemoteString = NULL;
	FARPROC pLoadLibrary;

	TCHAR strDllPath[MAX_PATH];
	GetFullPathName(dllName.c_str(), MAX_PATH, strDllPath, NULL);
	...
	try {
		...
		if (!WriteProcessMemory(hProc, pRemoteString, (void*)strDllPath, len * sizeof(wchar_t), 0)) {
...
{
	DWORD dwThisProgram = GetCurrentProcessId();
	...
	while (hRes)
	{
		if ((lstrcmp(pEntry.szExeFile, filename) == 0) && (pEntry.th32ProcessID != dwThisProgram))
		{
...
int CInjector::InjectAuto(std::wstring dllName, std::wstring procPath, DWORD* pPID)
{
	// check if already running
	std::wstring procName = StripPath(procPath);

	// kill PEL.exe
	int pId = GetProcessIdByName(procName.c_str());
	if (pId != 0)
	{
		/*MessageBox(NULL, _T("PEL.exe is already running. Please close it and try again!"), _T("Error"), MB_OK);
		TerminateProcess(pi.hProcess, 0);
		return -1;*/
		killProcessByName((TCHAR*)procName.c_str());
	}

	// kill CsvExport.exe
	const TCHAR* pProg = _T("CsvExport.exe");
	pId = GetProcessIdByName(pProg);
	if (pId != 0)
	{
		/*MessageBox(NULL, _T("PEL.exe is already running. Please close it and try again!"), _T("Error"), MB_OK);
		TerminateProcess(pi.hProcess, 0);
		return -1;*/
		killProcessByName(pProg);
	}
	...
	std::wstring dllDirectory = StripFile(modulePath);
	std::wstring dllPath = dllDirectory + dllName;

	try {
		std::wstring exeDirectory = StripFile(procPath);
		SetCurrentDirectoryW(exeDirectory.c_str());

		// Create the process
		if (!CreateProcessW(0,
			const_cast<LPWSTR>(procPath.c_str()),
			0,
			0,
			false,
			CREATE_SUSPENDED,
		...
		{
			MessageBox(NULL, _T("Can not find PEL.exe path"), _T("Error"), MB_OK);
			TerminateProcess(pi.hProcess, 0);
			return -1;
		}
		...
		*pPID = pi.dwProcessId;
		...
		std::wstring processName = StripPath(procPath);
